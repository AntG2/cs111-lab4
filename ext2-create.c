...
void write_block_bitmap(int fd)
{
	...
	int numBytes = LAST_BLOCK / 8;
	int remain = LAST_BLOCK % 8;
	int i = 0;
        for (; i < numBytes; i++)
	{
	  map_value[i] = 0xff;
	}
	u8 last = 0xff >> (8 - remain);
	map_value[i] = last;

	for (int j = NUM_BLOCKS / 8; j < BLOCK_SIZE; j++)
	{
	  map_value[j] = 0xff;
	}
...
void write_inode_bitmap(int fd)
{
	...
	int lastIndex = LAST_INO;
	int numBytes = lastIndex / 8;
	int remain = lastIndex % 8;
	int i = 0;
        for (; i < numBytes; i++)
	{
	  map_value[i] = 0xff;
	...
	for (int j = NUM_INODES / 8; j < BLOCK_SIZE; j++)
	{
	  map_value[j] = 0xff;
	}
